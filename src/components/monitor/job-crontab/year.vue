...
<script setup>
  ...
  const cycleTotal = computed(() => {
    const start = props.check(cycle01.value, fullYear, maxFullYear - 1)
    const end = props.check(cycle02.value, start + 1, maxFullYear)
    return start + '-' + end
  })
  const averageTotal = computed(() => {
    const start = props.check(average01.value, fullYear, maxFullYear - 1)
    const step = props.check(average02.value, 1, 10)
    return start + '/' + step
  })
