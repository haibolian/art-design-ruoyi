...
<script setup>
  ...
  const cycleTotal = computed(() => {
    const start = props.check(cycle01.value, 1, 6)
    const end = props.check(cycle02.value, start + 1, 7)
    return start + '-' + end
  })
  const averageTotal = computed(() => {
    const week = props.check(average01.value, 1, 4)
    const day = props.check(average02.value, 1, 7)
    return day + '#' + week
  })
  const weekdayTotal = computed(() => props.check(weekday.value, 1, 7) + 'L')
