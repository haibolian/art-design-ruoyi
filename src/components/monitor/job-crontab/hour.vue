...
<script setup>
  ...
  const cycleTotal = computed(() => {
    const start = props.check(cycle01.value, 0, 22)
    const end = props.check(cycle02.value, start + 1, 23)
    return start + '-' + end
  })
  const averageTotal = computed(() => {
    const start = props.check(average01.value, 0, 22)
    const step = props.check(average02.value, 1, 23 - start)
    return start + '/' + step
  })
