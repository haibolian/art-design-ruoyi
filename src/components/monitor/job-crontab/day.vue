...
<script setup>
  ...
  const cycleTotal = computed(() => {
    const start = props.check(cycle01.value, 1, 30)
    const end = props.check(cycle02.value, start + 1, 31)
    return start + '-' + end
  })
  const averageTotal = computed(() => {
    const start = props.check(average01.value, 1, 30)
    const step = props.check(average02.value, 1, 31 - start)
    return start + '/' + step
  })
  const workdayTotal = computed(() => props.check(workday.value, 1, 31) + 'W')
